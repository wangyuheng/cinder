...
- **Codex 集成**：集成 OpenAI Codex 提供更强大的代码生成能力
  - 自动使用 Codex 进行代码生成
  - Soul profile 上下文传递
  - 自动降级到 CodeGenerator
  - 支持所有 Codex CLI 参数
...
### 📊 LLM/Agent 可观测性（v3.2.0 新增）

Cinder 现已集成 Phoenix (Arize AI) 提供完整的 LLM 和 Agent 可观测性：

- **OpenTelemetry 追踪**：使用 OpenTelemetry 标准进行分布式追踪
- **LLM 调用追踪**：记录所有 LLM 调用的详细信息
  - Prompt 和 Response 内容
  - Token 使用统计（输入、输出、总计）
  - 模型参数和配置
  - 调用时长和成本估算
- **Agent 执行追踪**：按任务聚合的分层追踪
  - Task 级别的聚合 span
  - Phase 级别的执行阶段 span
  - LLM 级别的调用 span
- **OpenInference 语义约定**：遵循 OpenInference 标准的属性命名
- **Phoenix 可视化**：通过 Phoenix UI 查看和分析追踪数据
  - 分层的 trace 结构
  - LLM input/output messages
  - Token 使用统计
  - 执行时间分析

详见 [可观测性文档](docs/OBSERVABILITY.md)。
...
### 7. Codex 集成（v3.1.0 新增）

Cinder 现已集成 OpenAI Codex，提供更强大的代码生成能力。
...
```bash
# 自动使用 Codex（如果启用）
cinder execute "创建一个简单的 Python 函数"

# 直接使用 Codex CLI
codex exec "Create a REST API"
```

详见 [Codex 集成文档](docs/CODEX_INTEGRATION.md)。
...
详见 [迁移指南](docs/ARCHITECTURE_MIGRATION_GUIDE.md)。
...
# 查看所有任务（tasks.py 已移至 scripts/ 目录）
inv --list
